...
xfree (void * addr)
{
    ...
    if (curr == NULL)
    {
        fprintf (stderr, "ERROR: memory list is empty (addr %p)\n", user_addr);
        return;
    }
    ...
    fprintf (stderr, "ERROR: addr %p not malloc-ed\n", user_addr);
}

/*
 * writes all info of the unallocated memory and open files
 */
extern void
...
xfclose (FILE * fptr)
{
    ...
    if (file_counter < 0)
    {
        fprintf (stderr, "ERROR: trying to close an unopened file.\n");
        return (-1);
    }
    return (fclose (fptr));
}
